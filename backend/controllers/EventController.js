    ...
    try {
        ...
        if (event.status === 'draft') {
            ...
            if (regDeadline) {
                ...
                if (newDeadline > new Date(event.regDeadline)) {
                    event.regDeadline = regDeadline;
                }
            }
            if (regLimit !== undefined) {
                const newLimit = parseInt(regLimit);
                if (newLimit === 0 || newLimit >= event.regLimit) {
                    event.regLimit = newLimit;
                }
            }
